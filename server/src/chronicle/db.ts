...
CREATE INDEX IF NOT EXISTS idx_war_events_planet_id ON war_events(planet_id);
  ...
  return {
    ...
    listPlanets: (q: PlanetQuery) => { if (!ensureReady() || !db) return []; return db.prepare(`SELECT p.planet_id,COALESCE(MAX(p.planet_name),MAX(e.planet_name)) planet_name,MAX(p.timestamp) latest_timestamp,GROUP_CONCAT(DISTINCT p.campaign_type) campaign_types,COUNT(p.id) sample_count,(SELECT COUNT(*) FROM war_events we WHERE we.planet_id=p.planet_id) event_count FROM campaign_progress_log p LEFT JOIN war_events e ON e.planet_id = p.planet_id GROUP BY p.planet_id ORDER BY latest_timestamp DESC LIMIT ? OFFSET ?`).all([q.limit,q.offset]) as Array<Record<string, unknown>>; },
