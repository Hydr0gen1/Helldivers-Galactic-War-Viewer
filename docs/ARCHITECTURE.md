...
3. **Analyzer** sends a stripped snapshot projection to the configured AI provider every 5 minutes and caches the `Recommendation` JSON
...
- The provider API key never reaches the client bundle
...
- AI provider failures keep the previous recommendation with `degraded: true`
- Bad AI JSON goes through `repair.ts` before keeping previous
